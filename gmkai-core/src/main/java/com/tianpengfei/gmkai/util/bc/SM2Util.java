...
public class SM2Util extends GMBaseUtil {
     ...
     * 生成随机秘钥对
     *
     * @return 第一个byteArray为公钥<BR>
     * 第二个byteArray为私钥
     */
    public static byte[][] generateRawKeyPair(SecureRandom random) {

        AsymmetricCipherKeyPair key = BCECUtil.generateKeyPairParameter(DOMAIN_PARAMS, random);
        ...
        // 开头0x04 为非压缩标志
        keyPair[0] = publicKey.getEncoded(false);
        keyPair[1] = privateKey.toByteArray();
        return keyPair;
    }
