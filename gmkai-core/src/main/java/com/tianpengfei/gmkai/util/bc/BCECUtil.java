...
public class BCECUtil {
    ...
    private static final String PEM_STRING_PRIVATEKEY = "PRIVATE KEY";
    private static final String PEM_STRING_CERTIFICATE = "CERTIFICATE";
    ...
    public static ECPrivateKeyParameters convertRawPrivateKeyToParameters(byte[] priKey) {

        return BCECUtil.createECPrivateKeyParameters(priKey, SM2Util.DOMAIN_PARAMS);
    }

    public static byte[] convertPrivateKeyToRawPrivateKey(BCECPrivateKey priKey) {

        return priKey.getD().toByteArray();
    }

    public static BCECPrivateKey convertRawPrivateKeyToPrivateKey(byte[] priKey) {
        ECPrivateKeySpec privateKeySpec = new ECPrivateKeySpec(new BigInteger(1, priKey), SM2Util.JDK_EC_SPEC);
    ...
    /**
     * @param pubKey 带压缩相关标志
     * @return 公钥参数
     */
    public static ECPublicKeyParameters convertPublicKeySRawToParams(byte[] pubKey) {
        return new ECPublicKeyParameters(SM2Util.CURVE.decodePoint(pubKey), SM2Util.DOMAIN_PARAMS);
    }
     ...
     * @return 带压缩相关标志
     */
    public static byte[] convertPublicKeyParamsToSRaw(ECPublicKeyParameters pubKey) {
        return pubKey.getQ().getEncoded(false);
    }
     ...
     * @return 带压缩相关标志
     */
    public static byte[] convertPublicKeyToSRaw(BCECPublicKey pubKey) {
        return pubKey.getQ().getEncoded(false);
    }
     ...
     * @return 带压缩相关标志
     */
    public static BCECPublicKey convertSRawToPublicKey(byte[] pubKey) {

        ECPoint ecPoint = SM2Util.CURVE.decodePoint(pubKey);
        ECPublicKeySpec publicKeySpec = new ECPublicKeySpec(
                new java.security.spec.ECPoint(ecPoint.getXCoord().toBigInteger(), ecPoint.getRawYCoord().toBigInteger())
                , SM2Util.JDK_EC_SPEC);
    ...
    public static BCECPrivateKey convertPKCS8ToECPrivateKey(byte[] pkcs8Key)
        ...
        KeyFactory kf = KeyFactory.getInstance(ALGO_NAME_EC, new BouncyCastleProvider());
        return (BCECPrivateKey) kf.generatePrivate(peks);
    }

    public static ECPrivateKeyParameters convertPKCS8ToECParams(byte[] pkcs8Key)
            throws NoSuchAlgorithmException, InvalidKeySpecException {

        return convertPrivateKeyToParameters(convertPKCS8ToECPrivateKey(pkcs8Key));
    }
     ...
     * @throws IOException
     */
    public static String convertECPrivateKeyToPKCS8PEM(BCECPrivateKey privateKey) throws IOException {


        return convertEncodedDataToPEM(PEM_STRING_PRIVATEKEY, privateKey.getEncoded());
    }
    ...
    public static ECPublicKeyParameters convertX509ToECPublicKeyParams(byte[] x509Bytes) throws NoSuchProviderException, InvalidKeySpecException, NoSuchAlgorithmException {
        return convertPublicKeyToParameters(convertX509ToECPublicKey(x509Bytes));
    }
    ...
    public static String convertCertificationToPEM(byte[] encodedKey) throws IOException {
        return convertEncodedDataToPEM(PEM_STRING_CERTIFICATE, encodedKey);
    }
    ...
    public static BCECPrivateKey getPemPrivateKey(InputStream stream) throws IOException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {


        byte[] keyBytes = Streams.readAll(stream);

        String temp = new String(keyBytes);
        if (temp.contains("BEGIN EC PRIVATE KEY")) {
            String privKeyPEM = temp.replace("-----BEGIN EC PRIVATE KEY-----", "");
            privKeyPEM = privKeyPEM.replace("-----END EC PRIVATE KEY-----", "");
            return BCECUtil.convertSEC1ToBCECPrivateKey(Base64.decode(privKeyPEM));
        } else {

            return BCECUtil.convertPKCS8ToECPrivateKey(
                    BCECUtil.convertECPrivateKeyPEMToPKCS8(temp));
        }
