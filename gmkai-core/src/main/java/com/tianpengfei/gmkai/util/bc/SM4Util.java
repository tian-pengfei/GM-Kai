...
public class SM4Util extends GMBaseUtil {
    ...
    public static final String ALGORITHM_NAME_CBC_SSL3PADDING = "SM4/CBC/SSL3Padding";
    ...
    public static byte[] encrypt_CBC_SSL3Padding(byte[] key, byte[] iv, byte[] data)
            throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException,
            NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException,
            InvalidAlgorithmParameterException {
        Cipher cipher = generateCBCCipher(ALGORITHM_NAME_CBC_NOPADDING, Cipher.ENCRYPT_MODE, key, iv);

        int paddingLength = cipher.getBlockSize() -
                (data.length + 1) % cipher.getBlockSize();
        ByteBuffer m = ByteBuffer.allocate(data.length + 1 + paddingLength);
        m.put(data);
        for (int i = 0; i < paddingLength + 1; i++) {
            m.put((byte) paddingLength);
        }
        return cipher.doFinal(m.array());
    }

    public static byte[] decrypt_CBC_SSL3Padding(byte[] key, byte[] iv, byte[] cipherText)
            throws IllegalBlockSizeException, BadPaddingException, InvalidKeyException,
            NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException,
            InvalidAlgorithmParameterException {
        ...
        byte[] _plainText = cipher.doFinal(cipherText);

        int paddingLength = _plainText[_plainText.length - 1];
        ByteBuffer m = ByteBuffer.wrap(_plainText);

        byte[] plainText = new byte[_plainText.length - 1 - paddingLength];
        m.get(plainText);
        return plainText;
    }
