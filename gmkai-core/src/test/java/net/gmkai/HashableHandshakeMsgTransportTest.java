...
public class HashableHandshakeMsgTransportTest {
    ...
    private static final List<String> datas = ImmutableList.of(
    ...
    private void setUp() throws IOException {
        ...
        when(handshakeMsgTransport.readHandshakeMsg()).
                thenReturn(new TLSText(ContentType.HANDSHAKE, ProtocolVersion.TLCP11, addedClientFinishedData));

        hashableHandshakeMsgTransport = new HashableHandshakeMsgTransport(handshakeMsgTransport);

        for (String data : datas) {
            hashableHandshakeMsgTransport.writeHandshakeMsg(Hexs.decode(data));
        }
    ...
    public void should_get_hash_after_write_data() throws IOException {
        hashableHandshakeMsgTransport.writeHandshakeMsg(addedClientFinishedData);
